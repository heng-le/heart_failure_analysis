...
pc.data <- as.data.frame(pc_vsd$x)
df_pc <- cbind(clindata, pc.data) # Let's cbind to original df so we have metadata
head(df_pc)[1:10]
```

```{r}
ggplot(df_pc, aes(x = PC1, y = PC2, label = row.names(df_pc),color = age_cat))+
...
```{r}
plotPCA(vsd, intgroup=c("age_cat"))
```
```{r}
...
```{r}
head(clindata)
# there are only several possible factors of interest: age, sex, race, etiology 
clindata %>%
  group_by(sex, etiology) %>%
  dplyr::summarise(frequency = n()) 

# looking at the numbers, we think that the groups are relatively well-balanced for our diseases of interest
```
```{r}
ddsbat <- DESeqDataSetFromMatrix(raw_counts,
                                  colData = clindata,
                                  design = ~ etiology + sex)
ddsbat <- DESeq(ddsbat)
resultsNames(ddsbat)
...
#                 lab = rownames(resbat), 
#                 selectLab = c('CXCL9','ISG15','SERPING5','GBP5','CXCL10',  # top 5up DEG
#                               'H1-4','NCDN','PBX1','HTATSF1','TYROBP',  # top 5 down DEG
#                               'MX1','MX2','ACE2','BSG','CTSB'),          # genes of interest
#                 FCcutoff = 1, pCutoff = 0.05, 
#                 #ylim= c(0,10), xlim=c(-5,5),
#                 pointSize = 1.5, col=c('grey30', 'red3', 'red3', 'red3'),
#                 boxedLabels = TRUE,
#                 drawConnectors = TRUE,
#                 widthConnectors = 0.75,
#                 legendPosition = 'bottom',
#                 title = "postivity_COVID19_vs_Healthy",
#                 subtitle = "design ~ batch + gender + positivity") 
# 
